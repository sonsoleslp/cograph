...
`cograph` provides three functions for visualizing networks with grouped
structure:
...
multi-cluster layouts with summary edges, and
[`plot_mlna()`](http://sonsoles.me/cograph/reference/plot_mlna.md) for
multilevel networks rendered in 3D perspective. All three accept a
weight matrix and a list defining how nodes are assigned to groups,
clusters, or layers.
...
We use a 15-node weight matrix with three groups throughout this
vignette.
...
arranges groups of nodes into geometric patterns. For two groups it
produces a bipartite layout; for three or more it arranges groups along
the edges of a polygon or along arcs of a circle.

### Example 1: Polygon layout

With three groups, the default layout places each group along one edge
of a triangle.
...
The circular layout places groups along arcs of a circle instead of
polygon edges.
...
With two groups,
[`plot_htna()`](http://sonsoles.me/cograph/reference/plot_htna.md)
produces a bipartite layout by default. Jitter spreads nodes
horizontally based on their cross-group connectivity.
...
places each cluster inside a bordered shape and draws aggregated summary
edges between clusters. Individual within-cluster edges are shown inside
each shape. The alias
...
renders layers stacked in 3D perspective. Within-layer edges are drawn
as solid lines and between-layer edges as dashed lines connecting
adjacent layers. The alias
[`mlna()`](http://sonsoles.me/cograph/reference/plot_mlna.md) is
available as shorthand.
