...
This vignette demonstrates how to visualize statistical uncertainty on network edges using confidence interval underlays, p-value significance stars, and label templates.
...
Stars are shown using `edge_label_template` with the `{stars}` placeholder together with `edge_label_p` and `edge_label_stars = TRUE`.
...
CI width is shown as a translucent band behind each edge. Wider bands indicate more uncertainty.
...
Use `{low}` and `{up}` placeholders to show confidence interval bounds on edges.
